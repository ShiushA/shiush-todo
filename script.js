document.addEventListener("DOMContentLoaded", () => {
  ...
  function checkMissedTaskMovements() {
    ...
    // Check if we've crossed 11:59 PM (end of day)
    const hasCrossedEndOfDay = hasCrossedEndOfDayBoundary(lastOpened, now)

    // If we've crossed the end of day boundary, delete all completed tasks
    if (hasCrossedEndOfDay) {
      // 1. First delete all completed tasks
    ...
    // Process daily tasks
    if (daysPassed >= 1) {
      // 2. Move incomplete tasks to due section
      moveIncompleteTasks("today")

      // 3. Then overwrite today with tomorrow and clear tomorrow
      if (daysPassed === 1) {
        tasksCache.today = [...tasksCache.tomorrow]
        tasksCache.tomorrow = []
      } else {
        // If more than one day has passed, clear both today and tomorrow
        tasksCache.today = []
        tasksCache.tomorrow = []
      }
    }
  ...
  // New function to check if we've crossed the end of day (11:59 PM)
  function hasCrossedEndOfDayBoundary(startDate, endDate) {
    // If same day, no boundary crossed
    if (
    ...
    // Different days means we've crossed midnight
    return true
  }

  // New function to delete all completed tasks from all sections
  function deleteAllCompletedTasks() {
    const sections = ["today", "tomorrow", "week", "month", "year", "due"]

    sections.forEach((section) => {
      tasksCache[section] = tasksCache[section].filter((task) => !task.completed)
    })

    // Render if we're currently viewing an affected section
